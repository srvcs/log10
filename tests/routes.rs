...
use srvcs_log10::{api::Deps, health, router, telemetry};
...
/// genuinely exercises log10's validation branch (number -> proceed, non-number
/// -> 422) rather than rubber-stamping a fixed answer.
...
/// Extract the `result` field as an f64 for approximate comparison.
...
#[tokio::test]
async fn log10_of_thousand_is_three() {
    let isnumber = spawn_isnumber().await;
    let (status, body) = eval(&isnumber, json!(1000)).await;
...
#[tokio::test]
async fn log10_of_one_is_zero() {
    let isnumber = spawn_isnumber().await;
    let (status, body) = eval(&isnumber, json!(1)).await;
    assert_eq!(status, StatusCode::OK);
    assert!((result_f64(&body) - 0.0).abs() < 1e-9);
}

#[tokio::test]
async fn log10_of_float_input_is_fractional() {
    let isnumber = spawn_isnumber().await;
    // log10(4) ~= 0.602059991327962...
    let (status, body) = eval(&isnumber, json!(4.0)).await;
    assert_eq!(status, StatusCode::OK);
    assert!((result_f64(&body) - 0.602_059_991_327_962_4).abs() < 1e-9);
}

#[tokio::test]
async fn log10_of_tenth_is_minus_one() {
    let isnumber = spawn_isnumber().await;
    let (status, body) = eval(&isnumber, json!(0.1)).await;
    assert_eq!(status, StatusCode::OK);
    assert!((result_f64(&body) - (-1.0)).abs() < 1e-9);
}

#[tokio::test]
async fn rejects_zero_as_non_positive() {
    let isnumber = spawn_isnumber().await;
    let (status, body) = eval(&isnumber, json!(0)).await;
    assert_eq!(status, StatusCode::UNPROCESSABLE_ENTITY);
    assert_eq!(body["error"], "logarithm of a non-positive number");
}

#[tokio::test]
async fn rejects_negative_as_non_positive() {
    let isnumber = spawn_isnumber().await;
    let (status, body) = eval(&isnumber, json!(-5.0)).await;
    assert_eq!(status, StatusCode::UNPROCESSABLE_ENTITY);
    assert_eq!(body["error"], "logarithm of a non-positive number");
}
...
async fn rejects_when_isnumber_says_false() {
    ...
    let (status, _) = eval(&isnumber, json!(1000)).await;
    assert_eq!(status, StatusCode::UNPROCESSABLE_ENTITY);
}

#[tokio::test]
async fn forwards_isnumber_422() {
    // A leaf that rejects the input with 422 has its verdict propagated.
    let isnumber = spawn_mock(StatusCode::UNPROCESSABLE_ENTITY, json!({ "error": "nope" })).await;
    let (status, _) = eval(&isnumber, json!(1000)).await;
    assert_eq!(status, StatusCode::UNPROCESSABLE_ENTITY);
}
...
async fn degrades_when_isnumber_is_unreachable() {
    let (status, body) = eval(DEAD_URL, json!(1000)).await;
