use srvcs_log10::{api::Deps, config::Config, health, router, telemetry};
...
async fn main() {
    ...
    tracing::info!(addr = %cfg.bind_addr, env = %cfg.environment, "srvcs-log10 listening");
    let deps = Deps {
        isnumber_url: cfg.isnumber_url.clone(),
    };
